...
/// @title StringLeaderboard
/// @author Your Name
/// @notice A contract to manage a leaderboard of scores associated with string identifiers
...
contract StringLeaderboard is Authority {
    ...
    struct User {
        string user; // The user's string identifier
    ...
    event SubmitScore(string indexed user, uint64 indexed highScore); // Event emitted when a score is submitted but not added to the leaderboard
    event SubmitScoreAndAdd(string indexed user, uint64 indexed highScore); // Event emitted when a score is submitted and added to the leaderboard
    ...
    function submitScore(string memory user, uint64 highScore) public virtual onlyRole(SERVER_ROLE) {
    ...
    function _addToLeaderboard(string memory user, uint64 highScore, uint32 index) internal virtual {
    ...
    function reset() public virtual onlyRole(MANAGER_ROLE) {
        if (length() < 25_000) {
            delete leaderboard;
            emit Reset();
        }
