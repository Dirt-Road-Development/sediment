...
contract Leaderboard is Authority {
    ...
    event Reset();
    event SubmitScore(address indexed user, uint64 indexed highScore);
    event SubmitScoreAndAdd(address indexed user, uint64 indexed highScore);
    ...
    function _addToLeaderboard(address user, uint64 highScore, uint32 index) internal {
